defmodule Kiln.StoreTest do
  ...
  test "start_link exposes the supervised connection on a ready store", %{path: path} do
    ...
    assert [[4]] = Connection.query!(conn, "SELECT count(*) FROM schema_migrations")
    assert is_pid(conn)
  end
