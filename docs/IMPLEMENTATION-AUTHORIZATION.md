...
**Status:** Accepted
**Current result:** P0-W44 simplifies the implementation authorization model and re-binds `docs/authorizations/P1-S02-T01.authorization` to the further-amended Accepted-state plan digest against canonical decision base `2f88281527811b8c4be0243fb201ae4416730a13`. The simplification replaces source-topology authorization with semantic-scope authorization (capabilities, security/trust boundaries, durable contracts, ticket scope); the expected-files table is advisory by default; ordinary implementation decomposition including `lib/kiln/artifact/fs.ex` requires no additional authorization. The further-amended plan and re-bound record become active trusted Repository authority only when P0-W44 integrates on canonical `main`; until then the P0-W43 record remains active. No P1-S02 runtime implementation exists yet; P1-S02-T02 and later remain unauthorized
