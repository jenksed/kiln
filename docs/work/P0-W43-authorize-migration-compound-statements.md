...
- **P0-W43-R06:** Only governance documents that bind or reproduce the superseded digest, base, or authorization state shall change. A document that merely mentions T01 shall not be modified.
...
4. Update `docs/IMPLEMENTATION-AUTHORIZATION.md`, `AGENTS.md`, `README.md`, `docs/PLANNING.md`, `docs/IMPLEMENTATION-SLICES.md`, and `docs/ROADMAP.md` only where they bind or reproduce the superseded digest, base, or authorization state.
...
- **P0-W43-AC05 — Governance references are consistent and minimal**
  - **Given** the governance documents;
  - **When** each proposed edit is checked against its existing contract;
  - **Then** every document that binds or reproduces the superseded digest, base, or authorization state is corrected, and no document is modified merely for mentioning T01.
  - **Evidence:** per-file justification and final diff.
...
| P0-W43-E05 | AC05 | per-file governance justification and final diff |
